// laizhangsetu 插件前端(接口库 v4): Vue3 展示层, 全部经 ctx.invoke 走接口库。
// 图片本地缓存由主系统代发: /api/plugins/laizhangsetu/cache/<file>
...
function mount(container, ctx) {
  const App = {
    ...
    methods: {
      ...
      async loadHistory() {
        try {
          const r = await ctx.invoke('laizhangsetu.history.list')
